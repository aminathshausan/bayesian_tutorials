...
x1 <- runif(n=N, min=1, max=20)
mean(x1)
X1 <- cbind(rep(1, N), x1) 
x2 <- runif(n=N, min=1, max=20) #rnorm(N, mean = 5, sd = 5)
mean(x2)
X2 <- cbind(rep(1, N), x2) 
...
sigma_grps <- rinvgamma(2, shape = nu0/2, scale =  si_sqrd * nu0/2). #check this
...
X1 <- cbind(rep(1, N), x1)  #grp1
x2 <- runif(n=N, min=1, max=10)
X2 <- cbind(rep(1, N), x2)  #grp2
X <- rbind(X1,X2) #design matrix

betas_grp1 <- c(5, 1) #c(160, 5)
betas_grp2 <- c(7, 2). #c(150, 6)

sigma_sqrd_gp2 <- 10

mu_y_grp1 <- X1 %*% betas_grp1
mu_y_grp2 <- X2 %*% betas_grp2
sigma_y_grp1 <- 10*diag(length(mu_y_grp1))
sigma_y_grp2 <- 10*diag(length(mu_y_grp1))   
...
data_lst <- list(data = data, betas_grp1 = betas_grp1 , betas_grp2 = betas_grp2,
                 sigma2_noise_grp1 = 10, sigma2_noise_grp2 =10)
save(data_lst, file = "dataHierLinRgress.RData")
#save.image("data_HierLinReg.RData") #saves entire envirnment
#save(data, betas_grp1, betas_grp2, sigma2_noise_grp1 = sigma_sqrd_gp2, 
#     sigma2_noise_grp2 =sigma_sqrd_gp2,
#     file = "dataHierLinRgress.RData") #saves multiple objects
